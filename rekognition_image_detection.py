...
from rekognition_service.rekognition_objects import (
    RekognitionFace, RekognitionCelebrity, RekognitionLabel,
    RekognitionModerationLabel, RekognitionText, show_bounding_boxes, show_polygons, crop_bounding_boxes)
...
def run_reko_on_s3(image_name, bytes_ex):

    rekognition_client = boto3.client('rekognition')    
    s3_object = boto3.resource('s3').Object('1hackathonbucket',image_name)
    rekog_image = RekognitionImage.from_bucket(s3_object, rekognition_client)
    labels = rekog_image.detect_moderation_labels()
    print(f"Found {len(labels)} moderation labels.")
    all_labels = [label.to_dict() for label in labels]
    faces = rekog_image.detect_faces()
    all_faces = [face.to_dict() for face in faces]
    face_image = crop_bounding_boxes(bytes_ex, [[face.bounding_box for face in faces]],['aqua'])
    return {'labels':all_labels, 'faces':all_faces}, face_image
...
#     run_reko_on_s3('test_image_pol.jpg')